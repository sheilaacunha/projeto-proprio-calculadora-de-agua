...
function calcular() {
    ...
    if (inputIdade.value <= 0 || inputPeso.value <= 0) {
        ...
        let qtdAgua = ((40 * inputPeso.value) / 1000).toFixed(3)
        console.log(qtdAgua);

        divMsg.innerHTML = `<span>Quantidade recomendada por kg é de ${qtdAgua} litros por dia !!!!</span> `
    } else if (inputIdade.value >= 18 && inputIdade.value <= 55) {
        let qtdAgua = ((35 * inputPeso.value) / 1000).toFixed(3)
        console.log(qtdAgua);
        divMsg.innerHTML = `<span>Quantidade recomendada por kg é de ${qtdAgua} litros por dia !!!!</span> `

    } else if (inputIdade.value > 55 && inputIdade.value <= 65) {
        let qtdAgua = ((30 * inputPeso.value) / 1000).toFixed(3)
        console.log(qtdAgua);
        divMsg.innerHTML = `<span>Quantidade recomendada por kg é de ${qtdAgua} litros por dia !!!!</span> `
    } else if (inputIdade.value > 65) {
        let qtdAgua = ((25 * inputPeso.value) / 1000).toFixed(3)
        console.log(qtdAgua);
        divMsg.innerHTML = `<span>Quantidade recomendada por kg é de ${qtdAgua} litros por dia !!!!</span>`
